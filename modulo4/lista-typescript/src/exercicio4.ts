...
function markentingPresencial (pessoas: Colaboradores[]): Colaboradores[]{
    return pessoas.filter((item)=>{return item.setor === SETORES.MARKETING}).filter((item)=>{
        return item.presencial
    })
}
...
console.table(markentingPresencial(arrayColaboradores))